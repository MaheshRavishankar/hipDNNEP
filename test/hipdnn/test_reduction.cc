...
// ReduceSum models
#ifndef REDUCE_SUM_ALL_AXES_MODEL_PATH
#define REDUCE_SUM_ALL_AXES_MODEL_PATH "./reduce_sum_all_axes.onnx"
#endif

#ifndef REDUCE_SUM_AXIS1_MODEL_PATH
#define REDUCE_SUM_AXIS1_MODEL_PATH "./reduce_sum_axis1.onnx"
#endif

#ifndef REDUCE_SUM_LAST_TWO_MODEL_PATH
#define REDUCE_SUM_LAST_TWO_MODEL_PATH "./reduce_sum_last_two.onnx"
#endif

#ifndef REDUCE_SUM_AXIS0_MODEL_PATH
#define REDUCE_SUM_AXIS0_MODEL_PATH "./reduce_sum_axis0.onnx"
#endif

// ReduceMax model
#ifndef REDUCE_MAX_AXIS1_MODEL_PATH
#define REDUCE_MAX_AXIS1_MODEL_PATH "./reduce_max_axis1.onnx"
#endif

// ReduceMin model
#ifndef REDUCE_MIN_AXIS1_MODEL_PATH
#define REDUCE_MIN_AXIS1_MODEL_PATH "./reduce_min_axis1.onnx"
#endif

class HipDNNReductionTest : public ::testing::Test {
 protected:
  void SetUp() override {
...
TEST_F(HipDNNReductionTest, ReduceSumAllAxes) {
  ...
  size_t n = 1 * 4 * 8 * 8;
  auto x = GenerateTestData(n, 0.1f, 0.01f);
  RunAndCompare(REDUCE_SUM_ALL_AXES_MODEL_PATH, shape, x, 1e-2f);
}

TEST_F(HipDNNReductionTest, ReduceSumAxis1) {
  const std::vector<int64_t> shape = {1, 4, 8, 8};
  size_t n = 1 * 4 * 8 * 8;
  auto x = GenerateTestData(n, 0.1f, 0.01f);
  RunAndCompare(REDUCE_SUM_AXIS1_MODEL_PATH, shape, x, 1e-3f);
}

TEST_F(HipDNNReductionTest, ReduceSumLastTwo) {
  const std::vector<int64_t> shape = {1, 4, 8, 8};
  size_t n = 1 * 4 * 8 * 8;
  auto x = GenerateTestData(n, -1.0f, 0.01f);
  RunAndCompare(REDUCE_SUM_LAST_TWO_MODEL_PATH, shape, x, 1e-2f);
}

TEST_F(HipDNNReductionTest, ReduceSumAxis0) {
  const std::vector<int64_t> shape = {2, 4, 8, 8};
  size_t n = 2 * 4 * 8 * 8;
  auto x = GenerateTestData(n, 0.5f, -0.002f);
  RunAndCompare(REDUCE_SUM_AXIS0_MODEL_PATH, shape, x, 1e-3f);
}

TEST_F(HipDNNReductionTest, ReduceMaxAxis1) {
  const std::vector<int64_t> shape = {1, 4, 8, 8};
  size_t n = 1 * 4 * 8 * 8;
  auto x = GenerateTestData(n, -5.0f, 0.05f);
  RunAndCompare(REDUCE_MAX_AXIS1_MODEL_PATH, shape, x);
}

TEST_F(HipDNNReductionTest, ReduceMinAxis1) {
  const std::vector<int64_t> shape = {1, 4, 8, 8};
  size_t n = 1 * 4 * 8 * 8;
  auto x = GenerateTestData(n, 5.0f, -0.05f);
  RunAndCompare(REDUCE_MIN_AXIS1_MODEL_PATH, shape, x);
}
