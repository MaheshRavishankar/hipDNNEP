...
// Test single conv: hipdnn.executable → func.call with tensor.empty for DPS output.
// Declaration appears after the function (matching graph-to-executable output).
//
...
//       CHECK:   %[[RESULT:.*]] = call @hipdnn_graph_0(%[[INPUT]], %[[WEIGHT]], %[[EMPTY]])
//  CHECK-SAME:     : (tensor<1x3x32x32xf32>, tensor<16x3x3x3xf32>, tensor<1x16x30x30xf32>)
//  CHECK-SAME:     -> tensor<1x16x30x30xf32>
//       CHECK:   return %[[RESULT]] : tensor<1x16x30x30xf32>
//
//       CHECK: func.func private @hipdnn_graph_0(
//  CHECK-SAME:   tensor<1x3x32x32xf32>, tensor<16x3x3x3xf32>,
//  CHECK-SAME:   tensor<1x16x30x30xf32> {bufferization.writable = true})
//  CHECK-SAME:   -> tensor<1x16x30x30xf32>
//
//   CHECK-NOT: torch
...
// Test two sequential convs: each hipdnn.executable gets its own func.call and tensor.empty.
// Declarations appear after the function (matching graph-to-executable output).
//
...
//       CHECK:   %[[R0:.*]] = call @hipdnn_graph_0(%[[INPUT]], %[[W0]], %[[EMPTY0]])
//  CHECK-SAME:     : (tensor<1x3x32x32xf32>, tensor<16x3x3x3xf32>, tensor<1x16x30x30xf32>)
//  CHECK-SAME:     -> tensor<1x16x30x30xf32>
//       CHECK:   %[[EMPTY1:.*]] = tensor.empty() : tensor<1x32x28x28xf32>
//       CHECK:   %[[R1:.*]] = call @hipdnn_graph_1(%[[R0]], %[[W1]], %[[EMPTY1]])
//  CHECK-SAME:     : (tensor<1x16x30x30xf32>, tensor<32x16x3x3xf32>, tensor<1x32x28x28xf32>)
//  CHECK-SAME:     -> tensor<1x32x28x28xf32>
...
//  CHECK-SAME:   -> tensor<1x16x30x30xf32>
//
//       CHECK: func.func private @hipdnn_graph_1(
//  CHECK-SAME:   tensor<1x16x30x30xf32>, tensor<32x16x3x3xf32>,
//  CHECK-SAME:   tensor<1x32x28x28xf32> {bufferization.writable = true})
//  CHECK-SAME:   -> tensor<1x32x28x28xf32>
//
//   CHECK-NOT: torch
//   CHECK-NOT: hipdnn.executable
...
func.func private @hipdnn_graph_1(
    !torch.vtensor<[1,16,30,30],f32>, !torch.vtensor<[32,16,3,3],f32>,
    !torch.vtensor<[1,32,28,28],f32> {bufferization.writable = true})
    -> !torch.vtensor<[1,32,28,28],f32>
